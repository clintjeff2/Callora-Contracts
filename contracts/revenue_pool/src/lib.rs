...
impl RevenuePool {
    ...
    pub fn init(env: Env, admin: Address, usdc_token: Address) {
        ...
        env.events().publish((Symbol::new(&env, "init"), admin), usdc_token);
    }

    /// Return the current admin address.
    pub fn get_admin(env: Env) -> Address {
        env.storage().instance().get(&Symbol::new(&env, ADMIN_KEY)).unwrap_or_else(|| panic!("revenue pool not initialized"))
    }
    ...
    pub fn distribute(env: Env, caller: Address, to: Address, amount: i128) {
        ...
        let usdc_address: Address = env.storage().instance().get(&Symbol::new(&env, USDC_KEY)).unwrap_or_else(|| panic!("revenue pool not initialized"));
