...
fn fuzz_deposit_and_deduct() {
    ...
    // Pre-fund vault and user so deposits/deducts work
    fund_vault(&usdc_admin, &vault_address, 1_000);
    vault.init(&owner, &usdc_address, &Some(1_000), &None, &None, &None);
    let mut expected: i128 = 1_000;
    ...
    for _ in 0..500 {
        ...
        if action == 0 {
            let amount: i128 = rng.gen_range(1..=500);
            fund_user(&usdc_admin, &owner, amount);
            approve_spend(&env, &usdc_client, &owner, &vault_address, amount);
            vault.deposit(&owner, &amount);
            expected += amount;
        } else if expected > 0 {
            let amount: i128 = rng.gen_range(1..=expected.min(500));
