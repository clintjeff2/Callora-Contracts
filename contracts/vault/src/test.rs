...
fn init_with_balance_emits_event() {
    ...
    let events = env.as_contract(&contract_id, || {
        CalloraVault::init(env.clone(), owner.clone(), usdc_token.clone(), Some(1000), None);
        env.events().all()
    });
...
fn set_admin_unauthorized_fails() {
    ...
    assert!(result.is_err(), "expected error when non-admin tries to set admin");
}
...
fn distribute_unauthorized_fails() {
    ...
    assert!(result.is_err(), "expected error when non-admin tries to distribute");
}
...
fn distribute_insufficient_usdc_fails() {
    ...
    assert!(result.is_err(), "expected error for insufficient USDC balance");
}
